...
<main>
    ...
    <section id="tendencias-section">
        ...
        $sqlTendencias = "SELECT DISTINCT * FROM VIDEOJUEGOS WHERE stock = (SELECT MIN(stock) FROM VIDEOJUEGOS WHERE stock > 0 AND precio < 30  AND 'id_plataforma = 80' LIMIT 1) LIMIT 1";
    ...
    <section id="novedades-section">
        ...
        <?php
        $sql = "SELECT id_videojuegos, titulo, imagen, precio FROM VIDEOJUEGOS WHERE fecha_lanzamiento >= '2021-01-01' AND id_plataforma = 80 ORDER BY RAND() DESC LIMIT 8";
        $result = mysqli_query($conexion, $sql);
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                echo '<form method="POST" action="../juego-detalle/juego-detalle.php" class="item-form">';
    ...
    <section id="filtros-juegos">
        ...
        <div class="filtro-juegos">
            ...
            $sqlFiltros = "SELECT * FROM VIDEOJUEGOS WHERE id_plataforma = 80";
            $sqlContar = "SELECT COUNT(*) as total FROM VIDEOJUEGOS WHERE id_plataforma = 80";
